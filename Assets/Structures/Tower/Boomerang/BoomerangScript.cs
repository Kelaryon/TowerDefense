...
public class BoomerangScript : MonoBehaviour,IProjectileInterface
{
    ...
    void Attack()
    {
        try
        {
            ...
            {
                target.IncDamage(damage);
                DamageModifier();
                target = parentTower.ReturnTargetLocator().FindClosestTargetInRange(target.transform.position, parentTower.GetBank().GetEnemyList(), 5f, 10f);
