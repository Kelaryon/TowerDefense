...
public class HolographicTower : MonoBehaviour
{
    ...
    {
        transform.position = Vector3.Lerp(transform.position, CellToWorld(bank.GetGridMap().CellGridPosition(Utils.GetMouseWorldPoint(),new Vector2Int(bank.GetBuilding().GetLength(), bank.GetBuilding().GetWidth()))), Time.deltaTime * 20f);
    ...
    public void Setup(Controller bank)
    {
        this.bank = bank;
    }
    private Vector3 CellToWorld(Vector3Int cellPos)
    {
        return bank.GetGridMap().GetCellWorldPosition(cellPos.x, cellPos.z) + bank.GetGridMap().CellSizeDifference();
    }
    private void ChangeColorOnInvalid(Vector3 wayPos,Controller bank)
    {

        if (bank.GetBuilding().BuildCondition(wayPos,bank.GetGridMap()))
        {
