...
public class Controller : MonoBehaviour
{
    ...
    private GridMapScript<Waypoint> gridMap;
    ...
    {
        mapRef.SetBank(this);
        currentBallance = mapData.StartBallance;
        UpadateDispalyGold();
        Time.timeScale = 1;
        gridMap = new GridMapScript<Waypoint>(mapData);
    }
    private void Start()
    {
    ...
    #region Enemy Management Link, Can be changed for Enemies to get their manager directly from SO RefObject 
    #endregion
    ...
    public GridMapScript<Waypoint> GetGridMap()
    {
        return gridMap;
    }
    ...
    private void SaveMap(GridMapScript<Waypoint> gridMap)
    {
        int lenght = gridMap.ReturnSizeX();
        int width = gridMap.ReturnSizeZ();
        HashSet<Structure> listOfStructures = new HashSet<Structure>();
        for (int i = 0; i < lenght; i++) {
            for (int j = 0; j < width; j++) {
                listOfStructures.Add(gridMap.GetCellValue(i, j).GetStructure);
            }
        }
    }
    #endregion
...
public class UpdateSystem
{
    ...
    {
        building1 = new MethodsInfo[] {null, new(Update1, "First Update", "First Update",true), null };
